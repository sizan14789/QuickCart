      ...
      {productsData.length != 0 ? (
        <>
          <h2 className="relative mb-6 self-start text-2xl md:text-4xl text-gray-700 font-medium ">
            {search ? `You searched for "${search}"` : "Products"}
            <p
              className={`w-full h-0.5 absolute bg-orange-600 ${
                search ? "hidden" : ""
              }`}
            ></p>
          </h2>
          {newProductsData().length!=0 ? (
            ...
            <h2 className="text-2xl my-auto font-medium text-gray-700 self-center" >Filtered too <span className="text-orange-600" >much</span>!!!</h2>
          )}
        </>
      ) : (
